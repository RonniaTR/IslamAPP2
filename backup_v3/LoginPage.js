...
export default function LoginPage() {
  ...
  // --- YENİ MİSAFİR MANTIĞI ---
  const handleGuestClick = async () => {
    const savedGuestId = localStorage.getItem('islamapp_guest_id');

    if (savedGuestId) {
      // Daha önce giriş yapmış → direkt içeri al
      setGuestLoading(true);
      const user = await loginAsGuest();
      if (user) navigate('/', { replace: true });
      setGuestLoading(false);
    } else {
      // İlk kez giriyor → isim sorma ekranını aç
  ...
  return (
    <div className="min-h-screen splash-bg flex flex-col items-center justify-center max-w-[430px] mx-auto px-6 relative overflow-hidden" data-testid="login-page">
      ...
      <div className="relative z-10 w-full space-y-3 animate-fade-in" style={{ animationDelay: '0.45s' }}>
        {/* Google Butonu — değişmedi */}
        <button
          onClick={handleGoogleLogin}
          data-testid="google-login-btn"
          disabled={googleLoading || guestLoading}
          className="w-full flex items-center justify-center gap-3 bg-[#F5F5DC] hover:bg-[#E8E8C8] text-[#0A1F14] font-semibold py-3.5 px-6 rounded-xl transition-all duration-200 shadow-lg active:scale-[0.98] disabled:opacity-60"
        >
          ...
          {googleLoading ? 'Yönlendiriliyor...' : (t.google_login || 'Google ile Giriş Yap')}
        </button>

        {/* --- MİSAFİR GİRİŞİ BÖLÜMÜ --- */}
        {showNameInput ? (
          <motion.div
          ...
          >
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && submitGuestName()}
              autoFocus
              className="w-full bg-[#070D18] border border-[#C8A55A]/30 text-[#EBE5D8] rounded-2xl p-4 outline-none focus:border-[#C8A55A] text-center placeholder-[#C8A55A]/40"
            />
            <button
              onClick={submitGuestName}
              disabled={guestLoading || !guestName.trim()}
              className="w-full py-4 rounded-2xl font-bold text-[#070D18] disabled:opacity-60 transition-all"
              style={{ background: 'linear-gradient(135deg, #C8A55A 0%, #A08030 100%)' }}
            >
              {guestLoading ? 'Hazırlanıyor...' : 'Maceraya Başla'}
            </button>
          </motion.div>
        ) : (
          <motion.button
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 px-6 rounded-xl transition-all duration-300 disabled:opacity-60"
            style={{
          ...
          >
            <User size={20} />
            {guestLoading ? 'Giriş yapılıyor...' : (t.guest_login || 'Misafir Olarak Devam Et')}
          </motion.button>
        )}
      </div>
