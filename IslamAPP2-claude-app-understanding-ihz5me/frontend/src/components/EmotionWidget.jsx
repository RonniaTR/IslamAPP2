      ...
      <div className="p-4 pb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px]" style={{ color: theme.textSecondary }}>Kalbine bir ayet, bir dua, bir yol</p>
          </div>
        </div>
        {selected && (
          <button onClick={() => { setSelected(null); setOpen(true); }}
            ...
            style={{ background: `${theme.textSecondary}12` }} aria-label="Sıfırla">
          ...
          <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-1 px-1">
            {EMOTIONS.map((e, i) => (
              <motion.button key={e.id}
                ...
                <span className="text-[9px] font-bold" style={{ color: theme.textPrimary }}>{e.label}</span>
              </motion.button>
            ))}
          </div>
        </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-4 pb-4 space-y-3">
              ...
              <div className="flex items-center gap-2 -mt-1">
                ...
                <span className="text-xs font-bold" style={{ color: selected.color }}>{selected.label}</span>
                <span className="text-[10px] ml-auto" style={{ color: theme.textSecondary }}>Sana özel 3 kart</span>
              </div>
              ...
              <div className="rounded-xl p-3.5 relative"
                ...
                <p className="text-[9px] font-black uppercase tracking-wider mb-1.5 flex items-center gap-1"
                  ...
                  <BookOpen size={10} /> Bir Ayet
                </p>
                <p className="text-sm italic leading-relaxed" style={{ fontFamily: 'Georgia, serif', color: theme.textPrimary }}>
                  {selected.verse.text}
                </p>
                <p className="text-[10px] mt-1.5 font-bold" style={{ color: selected.color }}>{selected.verse.source}</p>
              </div>
              ...
              <div className="rounded-xl p-3.5"
                ...
                <p className="text-[9px] font-black uppercase tracking-wider mb-1.5 flex items-center gap-1"
                  ...
                  <Sparkles size={10} /> Bir Dua
                </p>
                <p className="text-sm leading-relaxed" style={{ color: theme.textPrimary }}>
                  {selected.dua}
                </p>
                <p className="text-[10px] mt-1.5 font-bold" style={{ color: theme.gold }}>{selected.duaSrc}</p>
              </div>
              ...
              {article && (
                <button onClick={() => navigate('/library')}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[9px] font-black uppercase tracking-wider" style={{ color: '#ffd369' }}>Sana Bir Yol</p>
                    <p className="text-xs font-bold truncate" style={{ color: '#f7e6ae' }}>{article.title}</p>
                  </div>
