...
export default function RapidQuiz({ theme, onXP }) {
  ...
  const finish = useCallback((finalXp) => {
    ...
    if (finalXp > 0) onXP(finalXp, 'game_quiz', 'Hızlı Bilgi');
  }, [onXP]);
  ...
  }, [flash, q, idx, questions.length, xp, finish]);

  if (phase === 'idle') {
    return (
      <div className="flex flex-col items-center justify-center px-6 py-10 text-center">
        <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-5" style={{ background: `${theme.gold}18` }}>
          <Zap size={36} style={{ color: theme.gold }} />
        </div>
        <h2 className="text-xl font-black mb-2" style={{ color: theme.textPrimary }}>Hızlı Bilgi</h2>
        <p className="text-sm mb-6 max-w-xs" style={{ color: theme.textSecondary }}>60 saniyede ne kadar çok soruyu doğru bilirsen o kadar XP! Doğru cevap = puanı kadar XP.</p>
  ...
  if (phase === 'done') {
    return (
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <h2 className="text-2xl font-black mb-1" style={{ color: theme.gold }}>+{xp} XP</h2>
        <p className="text-sm mb-6" style={{ color: theme.textSecondary }}>{correct} doğru cevap</p>
  ...
  return (
    <div className="px-4 w-full max-w-md mx-auto">
      ...
      <div className="h-1.5 rounded-full overflow-hidden mb-6" style={{ background: `${theme.textSecondary}20` }}>
        <div className="h-full rounded-full transition-all duration-1000 ease-linear" style={{ width: `${(timeLeft / DURATION) * 100}%`, background: theme.gold }} />
      </div>
