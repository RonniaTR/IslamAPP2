...
function WordPopup({ word, analysis, onClose, theme }) {
    ...
    <motion.div initial={{ opacity: 0, scale: 0.9, y: -5 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9 }}
      ...
      <div className="relative max-w-xs w-full rounded-2xl p-4 shadow-2xl"
        ...
        <div className="space-y-2">
          {analysis.root && <InfoRow label="Kök" value={analysis.root} theme={theme} />}
          {analysis.pattern && <InfoRow label="Kalıp (Vezin)" value={analysis.pattern} theme={theme} />}
          {analysis.meaning && <InfoRow label="Anlam" value={analysis.meaning} theme={theme} />}
          {analysis.grammar && <InfoRow label="Gramer" value={analysis.grammar} theme={theme} />}
          {analysis.quran_frequency != null && (
            <InfoRow label="Kur'an'daki Kullanım" value={`${analysis.quran_frequency} ayet`} theme={theme} />
          )}
        </div>
      </div>
    </motion.div>
...
function RaviChain({ chain, theme }) {
  ...
  return (
    <div className="mt-3 p-3 rounded-xl" style={{ background: `${theme.gold}08`, border: `1px solid ${theme.gold}20` }}>
      <p className="text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: theme.gold }}>
        <Link2 size={10} className="inline mr-1" /> İsnad Zinciri
      </p>
      ...
      {chain.grade && (
        <p className="text-[9px] mt-1.5" style={{ color: theme.textSecondary }}>
          Hüküm: <span style={{ color: GRADE_COLORS[chain.grade] || theme.textPrimary }}>{chain.grade_label || chain.grade}</span>
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="animate-fade-in" data-testid="hadith-page" style={{ background: theme.bg }}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="max-w-xl text-sm" style={{ color: theme.textSecondary }}>
              Kelime analizi · Ravi zinciri · AI açıklama · Sıhhat derecesi
            </p>
          </div>
          <div className="hidden sm:grid grid-cols-2 gap-3 text-right">
            <div className="p-2 rounded-xl" style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}` }}>
              <p className="text-[10px] uppercase tracking-widest" style={{ color: theme.textSecondary }}>Toplam</p>
              <p className="text-2xl font-semibold" style={{ color: theme.textPrimary }}>{hadiths.length}</p>
            </div>
            <div className="p-2 rounded-xl" style={{ background: theme.cardBg, border: `1px solid ${theme.cardBorder}` }}>
              <p className="text-[10px] uppercase tracking-widest" style={{ color: theme.textSecondary }}>Tema</p>
      ...
      <div className="px-4 mb-3 flex items-center gap-2">
        <span className="text-[10px]" style={{ color: theme.textSecondary }}>AI Detay:</span>
        {[{ id: 'ozet', label: 'Özet' }, { id: 'orta', label: 'Orta' }, { id: 'akademik', label: 'Akademik' }].map(dl => (
      ...
      <div className="px-4 pb-8 space-y-4">
        {featuredHadith && !loading && (
          <div className="rounded-2xl p-4" style={{ background: theme.cardBg, border: `1px solid ${theme.gold}30` }}>
            <div className="flex items-center justify-between gap-3 mb-3">
              <div>
                <p className="text-[10px] uppercase tracking-widest" style={{ color: theme.textSecondary }}>Öne Çıkan Hadis</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-14">
            ...
            <p className="text-sm" style={{ color: theme.textPrimary }}>Aramana uyan hadis bulunamadı.</p>
            <p className="mt-1 text-xs" style={{ color: theme.textSecondary }}>Farklı bir tema, kaynak veya kısa ifade dene.</p>
          </div>
        ) : (
              ...
              <motion.article key={h.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                      ...
                      {h.explanation && (
                        <div className="mt-3 rounded-xl p-3" style={{ background: theme.inputBg }}>
                          <p className="text-[10px] uppercase tracking-widest mb-1" style={{ color: theme.textSecondary }}>Kısa Not</p>
                      ...
                      {/* Ravi Chain */}
                      {raviData[h.id] ? (
                        <RaviChain chain={raviData[h.id]} theme={theme} />
                      ) : (
                      ...
                      {aiExplain[h.id] ? (
                        <div className="mt-3 rounded-xl p-3" style={{ background: `${theme.gold}08`, border: `1px solid ${theme.gold}15` }}>
                          <div className="flex items-center gap-1 mb-1.5">
                            ...
                            <span className="text-[10px] font-bold" style={{ color: theme.gold }}>AI Açıklama</span>
                          </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <button onClick={() => handleSave(h)} disabled={savingId === h.id}
                    ...
                    {savedId === h.id ? 'Kaydedildi' : 'Kaydet'}
                  </button>
                </div>
              </motion.article>
            ))}
      ...
      {/* Word Analysis Popup */}
      <AnimatePresence>
        {wordPopup && <WordPopup word={wordPopup.word} analysis={wordPopup.analysis} onClose={() => setWordPopup(null)} theme={theme} />}
      </AnimatePresence>
    </div>
  );
}
