...
const DHIKR_LIST = [
  { id: 'subhanallah', arabic: 'سُبْحَانَ اللَّهِ', turkish: 'Sübhanallah', nameEn: 'Subhanallah', meaning: "Allah'ı tüm noksanlıklardan tenzih ederim", meaningEn: 'I glorify Allah, free of all imperfection', recommended: 33, color: '#10B981' },
  { id: 'elhamdulillah', arabic: 'الْحَمْدُ لِلَّهِ', turkish: 'Elhamdülillah', nameEn: 'Alhamdulillah', meaning: "Hamd Allah'a aittir", meaningEn: 'All praise belongs to Allah', recommended: 33, color: '#3B82F6' },
  { id: 'allahuekber', arabic: 'اللَّهُ أَكْبَرُ', turkish: 'Allahu Ekber', nameEn: 'Allahu Akbar', meaning: 'Allah en büyüktür', meaningEn: 'Allah is the greatest', recommended: 33, color: '#8B5CF6' },
  { id: 'lailaheillallah', arabic: 'لَا إِلَٰهَ إِلَّا اللَّهُ', turkish: 'Lâ ilâhe illallah', nameEn: 'La ilaha illallah', meaning: "Allah'tan başka ilah yoktur", meaningEn: 'There is no god but Allah', recommended: 100, color: '#F59E0B' },
  { id: 'estagfirullah', arabic: 'أَسْتَغْفِرُ اللَّهَ', turkish: 'Estağfirullah', nameEn: 'Astaghfirullah', meaning: "Allah'tan bağışlanma dilerim", meaningEn: 'I seek forgiveness from Allah', recommended: 100, color: '#EF4444' },
  { id: 'salavat', arabic: 'اللَّهُمَّ صَلِّ عَلَى سَيِّدِنَا مُحَمَّدٍ', turkish: 'Salavat-ı Şerife', nameEn: 'Salawat', meaning: "Peygamberimize salat ve selam", meaningEn: 'Blessings and peace upon our Prophet', recommended: 100, color: '#C8A55A' },
  { id: 'hasbunallah', arabic: 'حَسْبُنَا اللَّهُ وَنِعْمَ الْوَكِيلُ', turkish: 'Hasbünallah', nameEn: 'Hasbunallah', meaning: 'Allah bize yeter, O ne güzel vekildir', meaningEn: 'Allah is sufficient for us; what an excellent Guardian', recommended: 7, color: '#06B6D4' },
];
...
export default function DhikrPage() {
  ...
  const { lang } = useLang();
  const tt = useTx();
  const dName = (d) => (lang === 'tr' ? d.turkish : (d.nameEn || d.turkish));
  const dMean = (d) => (lang === 'tr' ? d.meaning : (d.meaningEn || d.meaning));
      ...
      <div className="min-h-screen pb-24 flex flex-col" style={{ background: theme.bg }}>
        ...
        <div className="px-4 pt-5 pb-2 flex items-center justify-between">
          <button onClick={() => { reset(); setSelected(null); }} className="text-sm px-3 py-1.5 rounded-xl"
            style={{ color: theme.textSecondary, background: theme.surface }}>← {tt('Geri')}</button>
          <div className="text-center">
            <p className="text-xs" style={{ color: theme.textSecondary }}>{dName(selected)}</p>
          </div>
        ...
        <div className="text-center px-4 py-3">
          ...
          <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{dMean(selected)}</p>
        </div>
            ...
            <div className="text-center z-10">
              {completed ? (
                <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>
                  ...
                  <p className="text-lg font-bold" style={{ color: '#10B981' }}>{tt('Tamamlandı!')}</p>
                </motion.div>
              ) : (
                <>
        ...
        <div className="px-6 pb-4 space-y-3">
          {completed && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="text-sm font-semibold" style={{ color: '#10B981' }}>{tt('Maşallah!')} {target} {tt('zikir tamamlandı')} 🤲</p>
              <button onClick={reset} className="mt-3 px-6 py-2 rounded-xl text-sm font-semibold"
                style={{ background: `${theme.gold}20`, color: theme.gold }}>{tt('Tekrar Başla')}</button>
            </motion.div>
          )}
          <p className="text-center text-xs" style={{ color: theme.textSecondary }}>{tt('Ekrana dokunarak zikir çek')}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pb-24" style={{ background: theme.bg }}>
      <div className="px-5 pt-6 pb-3">
        <h1 className="text-2xl font-bold" style={{ fontFamily: 'Playfair Display, serif', color: theme.textPrimary }}>{tt('Zikir')}</h1>
        <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{tt('Günlük tesbih ve zikir takibi')}</p>
      </div>

      {/* Stats */}
      <div className="flex gap-3 px-5 mb-5">
        {[
          { label: tt('Bugün'), value: stats.today_total, icon: '📿' },
          { label: tt('Seri'), value: `${stats.streak} ${tt('gün')}`, icon: '🔥' },
          { label: tt('Haftalık'), value: stats.weekly_total, icon: '📊' },
        ].map((s, i) => (
      ...
      <div className="mx-5 mb-4 rounded-2xl p-4 text-center" style={{ background: `${theme.gold}08`, border: `1px solid ${theme.gold}15` }}>
        ...
        <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{tt("Kalpler ancak Allah'ı anmakla huzur bulur (Ra'd 28)")}</p>
      </div>
          ...
          <motion.button key={d.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <p className="font-semibold text-sm" style={{ color: theme.textPrimary }}>{dName(d)}</p>
                <p className="text-sm" style={{ fontFamily: 'Amiri, serif', color: theme.gold, direction: 'rtl' }}>{d.arabic}</p>
              </div>
              <p className="text-xs truncate" style={{ color: theme.textSecondary }}>{dMean(d)}</p>
            </div>
