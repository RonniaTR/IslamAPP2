    ...
    <div className="animate-fade-in" data-testid="ramadan-page">
      ...
      <div className="px-5 pt-6 pb-4" style={{ background: 'linear-gradient(180deg, rgba(212,175,55,0.12) 0%, transparent 100%)' }}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <p className="text-xs tracking-widest uppercase" style={{ color: theme.gold }}>{t.ramadan_mubarak || tt('Ramazan Mübarek')}</p>
        </div>
        <h1 className="text-2xl font-bold" style={{ color: theme.textPrimary, fontFamily: 'Playfair Display, serif' }}>{t.ramadan || tt('Ramazan')}</h1>
      </div>
      ...
      {iftarCountdown && (
        <div className="mx-4 mb-4 rounded-2xl p-5 text-center animate-pulse-gold" style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.15), rgba(212,175,55,0.05))', border: '1px solid rgba(212,175,55,0.25)' }} data-testid="ramadan-iftar">
          ...
          <p className="text-xs tracking-widest mb-2" style={{ color: theme.gold }}>{(t.iftar_remaining || tt('İftara Kalan Süre')).toUpperCase()}</p>
          <div className="flex justify-center gap-3">
            {[{ v: iftarCountdown.h, l: t.hour_short || tt('Saat') }, { v: iftarCountdown.m, l: t.min_short || 'Dk' }, { v: iftarCountdown.s, l: t.sec_short || 'Sn' }].map(({ v, l }) => (
      ...
      <div className="mx-4 mb-4 card-islamic rounded-2xl p-4" data-testid="ramadan-sahur">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.sahur_time || tt('Sahur Vakti')}</span>
        </div>
        <p className="text-xl font-bold" style={{ color: theme.textPrimary }}>{prayerTimes?.fajr || '--:--'}</p>
        {sahurCountdown && <p className="text-xs mt-1" style={{ color: theme.textSecondary }}>{t.remaining || tt('Kalan')}: {pad(sahurCountdown.h)}:{pad(sahurCountdown.m)}:{pad(sahurCountdown.s)}</p>}
      </div>
      ...
      <div className="mx-4 mb-4 card-islamic rounded-2xl p-4" data-testid="ramadan-dua">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.today_dua || tt('Bugünün Duası')}</span>
        </div>
      ...
      {randomVerse && (
        <div className="mx-4 mb-4 card-islamic rounded-2xl p-4" data-testid="ramadan-verse">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.today_verse || tt('Bugünün Ayeti')}</span>
          </div>
      ...
      {randomHadith && (
        <div className="mx-4 mb-4 card-islamic rounded-2xl p-4" data-testid="ramadan-hadith">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.today_hadith || tt('Bugünün Hadisi')}</span>
          </div>
      ...
      <div className="mx-4 mb-4 rounded-2xl p-4" style={{ background: 'linear-gradient(135deg, rgba(212,175,55,0.08), rgba(15,61,46,0.5))', border: '1px solid rgba(212,175,55,0.12)' }} data-testid="ramadan-sadaka">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.sadaka_reminder || tt('Sadaka Hatırlatıcısı')}</span>
        </div>
        <p className="text-sm leading-relaxed" style={{ color: theme.textSecondary }}>{t.sadaka_hadith || tt('"Sadaka verin; çünkü sadaka kıyamet günü sahibine gölge olur." - Hz. Muhammed (s.a.v.)')}</p>
      </div>
      ...
      <div className="mx-4 mb-6 card-islamic rounded-2xl p-4" data-testid="ramadan-teravih">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-sm font-semibold" style={{ color: theme.gold }}>{t.teravih_prayer || tt('Teravih Namazı')}</span>
        </div>
        <p className="text-sm leading-relaxed" style={{ color: theme.textSecondary }}>{t.teravih_desc || tt('Teravih namazı yatsı namazından sonra kılınır. 20 veya 8 rekat olarak kılınabilir. Her 4 rekatta bir dinlenme (teravih) yapılır.')}</p>
        {prayerTimes?.isha && <p className="text-xs mt-2" style={{ color: theme.gold }}>{t.isha_prayer || tt('Yatsı Namazı')}: {prayerTimes.isha}</p>}
      </div>
    </div>
  );
}
