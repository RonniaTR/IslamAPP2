...
export default function LoginPage() {
  ...
  const submitGuestName = async () => {
    if (!guestName.trim()) {
        alert(tt("Lütfen liderlik tablosu için bir isim giriniz."));
        return;
    }
    setLoading(true);
    try {
      ...
      alert(tt("Bağlantı hatası oluştu. Lütfen tekrar deneyin."));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (loginWithGoogle) {
         ...
         alert(tt("Google girişi şu an aktif değil."));
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-[#032212] flex flex-col items-center justify-center p-6 font-sans relative overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[#A8B5A0] text-sm">{tt('İlim ve İbadet Yolculuğunuz')}</p>
        </div>
        ...
        <div className="bg-[#1a3a2a]/40 border border-[#ffd369]/20 backdrop-blur-xl p-8 rounded-[32px] shadow-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="relative flex items-center py-2">
              ...
              <span className="flex-shrink-0 mx-4 text-[#A8B5A0] text-xs">{tt('veya')}</span>
              <div className="flex-grow border-t border-[#ffd369]/10"></div>
            </div>
            ...
            <AnimatePresence mode="wait">
              {showNameInput ? (
                ...
                >
                  <input 
                    type="text" 
                    placeholder={tt('Liderlik tablosu için adınız?')} 
                    value={guestName}
                  ...
                  >
                    {loading ? <Loader2 className="animate-spin" size={20}/> : (
                      <>{tt('Maceraya Başla')} <LogIn size={20} /></>
                    )}
                  </button>
                </motion.div>
              ) : (
